    ...
    <div class="flex justify-between h-20 items-center">
      ...
      <div class="hidden md:flex flex-grow items-center justify-center lg:justify-start lg:ml-12 space-x-6 lg:space-x-8">
        {#each links as link}
          <a
            href={link.href}
            class="text-neutral-600 hover:text-brand-orange font-medium transition-colors {page.url.pathname === link.href ? 'text-brand-red border-b-2 border-brand-red' : ''}"
          >
            {link.name}
          </a>
        {/each}
      ...
      <div class="hidden md:flex items-center gap-4">
        {#if user_logged}
          ...
          <button onclick={logout} class="px-4 py-2 text-sm font-semibold text-neutral-600 hover:text-brand-red transition-all">
            Cerrar Sesión
          </button>
        {:else}
      ...
      <div class="md:hidden flex items-center gap-1">
        <button 
          ...
          class="p-2 text-neutral-600 flex flex-col gap-1.5 focus:outline-none"
          aria-label="Menu"
        >
          <div class="w-7 h-1 bg-neutral-900 rounded-full transition-all {mobileMenuOpen ? 'rotate-45 translate-y-2' : ''}"></div>
          <div class="w-7 h-1 bg-neutral-900 rounded-full transition-all {mobileMenuOpen ? 'opacity-0' : ''}"></div>
          <div class="w-7 h-1 bg-neutral-900 rounded-full transition-all {mobileMenuOpen ? '-rotate-45 -translate-y-2' : ''}"></div>
        </button>
      </div>
    </div>
    ...
    <div class="md:hidden bg-white border-t border-neutral-100 shadow-xl animate-in slide-in-from-top duration-300">
      <div class="px-6 py-8 space-y-4">
        ...
        <div class="pt-6 mt-6 border-t border-neutral-100 space-y-3">
             {#if user_logged}
                <a href="/dashboard" onclick={() => mobileMenuOpen = false} class="block w-full text-center py-4 rounded-2xl text-brand-violet font-black border border-brand-violet/20">PANEL ADMIN</a>
                <button onclick={logout} class="w-full text-center py-4 rounded-2xl text-red-600 font-bold hover:bg-red-50">Cerrar Sesión</button>
             {:else}
